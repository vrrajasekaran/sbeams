...
#
# adds a --fudge tag to avoid errors when attempt to create table or
# constraint that already exists.  Should not use this when table design
# gets fixed
...
do
  ...
  for cfile in CREATETABLES POPULATE CREATECONSTRAINTS CREATE_MANUAL_CONSTRAINTS ADD_MANUAL_CONSTRAINTS CREATEINDEXES
  do
    ...
    then 
      ...
      then
        echo GO
        $SBEAMS/lib/scripts/Core/runsql.pl -u $user -p $pass -i --delimit GO -s ${mod}_${cfile}.${db} --fudge
      else
        $SBEAMS/lib/scripts/Core/runsql.pl -u $user -p $pass -i -s ${mod}_${cfile}.${db} --fudge
      fi
    ...
    then 
      ...
      then
        echo GO
        $SBEAMS/lib/scripts/Core/runsql.pl -u $user -p $pass -i --delimit GO -s ${mod}_${cfile}.sql --fudge
      else
        $SBEAMS/lib/scripts/Core/runsql.pl -u $user -p $pass -i -s ${mod}_${cfile}.sql --fudge
      fi
