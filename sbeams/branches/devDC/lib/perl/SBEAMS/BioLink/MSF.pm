...
#	$log->debug( "Clustal returned $out" );
		  ...
		  $alignment .= $line;
			...
			if ( !$first_char ) {
				...
				for my $char ( @line ) {
					$cnt++;
					$name = 0 if $char =~ /^\s$/;
#					$log->debug( "Does $char exist?" );
					if ( !$name && $char !~ /^\s+$/ ) {
						$first_char = $cnt;
#						$log->debug( "Calculated firstchar with $line" );
						last;
					}
				}
			}
#			$log->debug( "First char is at $first_char" );
...
#			$log->debug( "name is $name, seq is $seq ( " . length( $seq ) . " chars) for [$line]\n" );
...
#		$aligned_seqs{$acc} =~ s/\s//g;
		push @all_aligned, [ $acc, $aligned_seqs{$acc} ];
#		$log->debug("$acc => $aligned_seqs{$acc}");
#		$alignment .= "$acc => $aligned_seqs{$acc}\n";
	}

#  my $parsed_alignment = $self->parse_aln( aln => $aligned_seqs ); 
