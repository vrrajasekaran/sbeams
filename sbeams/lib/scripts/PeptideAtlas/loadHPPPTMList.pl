...
sub parse_list {
  ...
  for ( my $idx = $book->[1]->{minrow}; $idx <= $book->[1]->{maxrow}; $idx++ ) {
    ...
    next unless $row[0];
    last if $row[0] =~ /Column documentation/;
    next if $row[0] eq 'Field';
    next if $row[0] =~ /Please fill in/;
    if ( $row[0] eq 'Abstract' ) {
      $row[1] =~ s/\<describe your methodology here\>//g;
    }
  ...
  if ( $book->[2]->{label} ne 'ModificationsList' ) {
    if ( $opts->{force} ) {
      print STDERR "Allowing Mis-labeled info sheet $book->[2]->{label}";
    } else {
      die "Mis-labeled info sheet $book->[2]->{label}";
    }
  }
...
# Known fields that we'll store in the db
  my %db_fields;
  for my $f ( qw(protein_symbol uniprot_accession ptm_name keywords unimod_id local_sequence protein_name n_sites comment confidence gene_symbol pubmed_id sequence_pos ) ) {
    $db_fields{$f}++;
  }
  for ( my $idx = $book->[2]->{minrow}; $idx <= $book->[2]->{maxrow}; $idx++ ) {
    ...
    for ( my $idx = 0; $idx <= $#keys; $idx++ ) {
      $keys[$idx] = 'ptm_name' if $keys[$idx] eq 'modification_name';
      $keys[$idx] = 'sequence_pos' if $keys[$idx] eq 'sequence_position';
      next unless $db_fields{$keys[$idx]};
      if ( $keys[$idx] eq 'local_sequence' ) {
        $row[$idx] =~ s/\-//g;
        if ( $row[$idx] =~ / (\S) / ) {
          my $mod_aa = lc( $1 );
          $row[$idx] =~ s/ \S /$mod_aa/;
          ...
          $row[$idx] =~ s/-/$aa/;
          if ( $row[$idx] =~ /\[/ ) {
            print STDERR "Peptide $row[$idx] has multiple mods, skipping\n";
            next;
          }
        }
      } elsif ( $keys[$idx] eq 'protein_name' ) {
        $row[$idx] = substr( $row[$idx], 0, 255 );
      }
...
sub show_lists {
  my $sql = qq~
  SELECT ptm_list_id, title 
  FROM $TBAT_PTM_LIST
  WHERE record_status = 'N'
  ORDER BY ptm_list_id ASC
  ~;
...
sub check_list {
  ...
  my $sql = "SELECT title FROM $TBAT_PTM_LIST WHERE ptm_list_id = $list_id";
...
sub update_list {
  ...
  my $files_dir = $UPLOAD_DIR . '/AT_ptm_list';
  ...
  if ( $opts->{list_file} ) {
    ...
    if ( -e $list_file ) {
      ...
      my $sql = qq~
      SELECT original_file FROM $TBAT_PTM_LIST 
      WHERE ptm_list_id = $list_id
      ~;
...
sub delete_list {
  ...
  my $sql = "SELECT COUNT(*) FROM $TBAT_PTM_LIST WHERE ptm_list_id = $list_id";
  my $result = $sbeams->selectrow_arrayref( $sql );
  if ( $result->[0] ) {
    my $sql = "SELECT COUNT(*) FROM $TBAT_PTM_LIST_ENTRY WHERE ptm_list_id = $list_id";
    my $result = $sbeams->selectrow_arrayref( $sql );
    if ( $result->[0] ) {
      if ( !$opts->{force} ) {
        usage( "This domain protein list already has data, use --force option to purge and load new records" );
      } else {
        $sbeams->do( "DELETE FROM $TBAT_PTM_LIST_ENTRY WHERE ptm_list_id = $list_id" );
        return 1;
      }
    } else {
      return 1;
...
sub create_list {
  ...
  $list_info->{n_ptm} = scalar( @{$list_data->{list_proteins}} );
...
sub fill_table {
  ...
  for my $prot ( @{$list_proteins} ) {
    $prot->{protein_symbol} ||= '';
    $prot->{gene_symbol} ||= '';
    $prot->{ptm_list_id} = $list_id;

    $sbeams->updateOrInsertRow( insert => 1,
                           table_name  => $TBAT_PTM_LIST_ENTRY,
                           rowdata_ref => $prot, 
                               verbose => $VERBOSE,
                              testonly => $TESTONLY );
