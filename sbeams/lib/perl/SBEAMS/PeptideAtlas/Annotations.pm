...
sub loadBuildSpectra {
  ...
  while (my $line = <INFILE>) {
    ...
    $self->insertAnnotationsIdentification(
...
###############################################################################
# insertAnnotationsIdentification --
###############################################################################
sub insertAnnotationsIdentification {
  my $METHOD = 'insertAnnotationsIdentification';
  my $self = shift || die ("self not passed");
  my %args = @_;
  ...
  #### If not, INSERT it
  unless ($spectrum_id) {
    $spectrum_id = $self->insertAnnotationsRecord(
      sample_id => $sample_id,
      spectrum_name => $spectrum_name,
      proteomics_search_batch_id => $search_batch_id,
    );
  ...
  unless ($spectrum_identification_id) {
    $spectrum_identification_id = $self->insertAnnotationsIdentificationRecord(
      modified_peptide_instance_id => $modified_peptide_instance_id,
      spectrum_id => $spectrum_id,
      atlas_search_batch_id => $atlas_search_batch_id,
      probability => $probability,
...
} # end insertAnnotationsIdentification
...
###############################################################################
# insertAnnotationsRecord --
###############################################################################
sub insertAnnotationsRecord {
  my $METHOD = 'insertAnnotationsRecord';
  my $self = shift || die ("self not passed");
  my %args = @_;
...
#  my mz_intensitities = $self->getAnnotationsPeaks(
#    proteomics_search_batch_id => $search_batch_id,
#    spectrum_name => $spectrum_name,
...
# getAnnotationsPeaks --
###############################################################################
sub getAnnotationsPeaks {
  my $METHOD = 'getAnnotationsPeaks';
  my $self = shift || die ("self not passed");
  my %args = @_;
  ...
  if ( -e $mzXML_filename ) {
    ...
    if ($spectrum_name =~ /(\d+)\.(\d+)\.\d$/) {
      $spectrum_number = $1;
      $buffer .= "INFO: Annotations number is $spectrum_number<BR>\n";
    }

    #### If we have a spectrum number, try to get the spectrum data
    if ($spectrum_number) {
      $filename = "$PHYSICAL_BASE_DIR/lib/c/Proteomics/getAnnotations/".
        "getAnnotations $spectrum_number $mzXML_filename |";
    }
...
###############################################################################
# insertAnnotationsIdentificationRecord --
###############################################################################
sub insertAnnotationsIdentificationRecord {
  my $METHOD = 'insertAnnotationsIdentificationRecord';
  my $self = shift || die ("self not passed");
  my %args = @_;
...
} # end insertAnnotationsIdentificationRecord
