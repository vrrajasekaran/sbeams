package SBEAMS::SolexaTrans::Relational;
use strict;
use warnings;
use Carp;
use Data::Dumper;
...
########################################################################
# $Id: Relational.pm,v 1.17 2008/09/02 16:09:11 vcassen Exp $
# Class to model some database table functionality
########################################################################


use SBEAMS::SolexaTrans::DBHManager;
use DBI;
use XML::Simple;
...
our @RELATIONAL_CLASS_ATTRS=qw(tablename table_fields indexes uniques no_sql_changes _dependent_classes _dbh);
@CLASS_ATTRIBUTES=@RELATIONAL_CLASS_ATTRS;
@DEFAULTS = (tablename => '', table_fields => {}, indexes => [], uniques => [],
	     _dependent_classes=>[], dbh_info=>{});
...
sub db_name1 {
    ...
    my $dbh_info=$self->dbh_info or confess "no dbh_info for $self???";
    $self->dbh_info->{db_name};
}
...
#    warn "dsn is $dsn; dbh_info are ",Dumper($dbh_info);
...
sub store {
    ...
    my $primary_key=$self->primary_key or confess ref $self, ": no primary key defined";
