...
SBEAMS::SolexaTrans::Solexa_Analysis - Methods to analize a group of solexametrix files
SBEAMS::Solexa
...
sub make_R_CHP_file {
  ...
  # temp dir for storing R/shell scripts, use config value or fall back
	my $R_temp_dir = ( $SOLEXA_TMP_DIR ) ? "${SOLEXA_TMP_DIR}/R_CHP_RUNS/${file_name}_R_CHP" : 
                           "$PHYSICAL_BASE_DIR/tmp/Solexa/R_CHP_RUNS/${file_name}_R_CHP";
...
data <- ReadSolexa(filenames =  cel.file.name)
...
if [ \$STATUS == 0 ]
	...
	then
		...
		chgrp solexadata $out_R_CHP_file $out_chip_image
...
if [ \$STATUS == 0 ]
	...
	then
		echo "R Exit Status 0"
		chgrp solexadata $out_chip_image
...
sub parse_R_CHP_file {
	...
	my $r_chp_protocol_name = $self->get_solexa_r_chp_protocol();
	...
	my %transform_map = (
				'1000'=> sub{return $self->get_solexa_id ;},
	...
	my %update_keys = ( probe_set_id   => 0,
	 ...
	 print "\nTransferring $r_chp_file -> solexa_gene_intensity" if ($VERBOSE >0);
	 ...
	 $sbeams->transferTable(
		 ...
		 delimiter=>"\t",
		 skip_lines=>'2',
		 dest_PK_name=>'solexa_gene_intensity_id',
		 dest_conn=>$sbeams,
...
sub log_R_run {
  ...
  my $logdir = $self->get_solexa_log_dir();
	
  open OUT, ">>$logdir/SOLEXA_R_CHP_RUNS.log" || die "Can't open R_CHP log dir $!\n";
...
#Given a string of comma seperarted solexa_ids OR a project id (will use the ids as defualt) find the most recent annotation set ids
...
sub get_annotation_set_id {
	...
	my $arrays = $args{solexa_ids};
	
	my $project_id = $args{project_id};
	
	unless ( ($project_id =~ /^\d/ || $arrays =~ /^\d/) ){
		
		confess(__PACKAGE__ . "::$method Need to provide arugments 'solexa_ids' or 'project_id'h\n") 
    	}
	
	my $constriant = '';
	
	if ($arrays){							#if we just have ids make the constriant.  This will always be the defualt 
		$constriant = qq~ WHERE  afa.solexa_id IN ($arrays)
				~;
	}else{								#if we have a project id join on the solexa sample table and constrian to the project table
		$constriant = qq~ JOIN $TBST_SOLEXA_SAMPLE afs ON (afa.solexa_sample_id = afs.solexa_sample_id)
	...
	my $sql = qq~ select * from $TBST_SOLEXA_SAMPLE ~;
#	my $sql = qq~ 	SELECT MAX(solexa_annotation_set_id) AS annotation_set_id, afa.type_id
#			FROM $TBST_SOLEXA afa JOIN $TBST_SOLEXA_ANNOTATION_SET anno_set
#			ON afa.type_id = anno_set.slide_type_id
#			$constriant
#			GROUP BY afa.type_id
#		~;
...
###############################################################################
# get_r_chp_protocol
#Given a string of comma seperarted solexa_ids find the most common R_CHP protocol
#Return the R_CHP protocol.  
#Warning not setup to indicate if a group of ids have utilized a differnt set of R_CHP protocols
###############################################################################
...
#			FROM $TBST_SOLEXA_GENE_INTENSITY gi
#			WHERE gi.solexa_id IN ($arrays)
#		~;
...
sub find_slide_type_name {
	...
	unless ( @file_names ){
		confess(__PACKAGE__ . "::$method No solexa CEL file Names were provided\n") 
    }
...
##Clean up the file names and make a comma seperated string
	...
	my $sql = qq~   SELECT distinct st.name 
					FROM $TBST_SOLEXA_SAMPLE afa 
					JOIN $TBST_SOLEXA_RUN st ON (afa.type_id = st.slide_type_id)
...
#given a project_id find all the solexa ids that have data
#return array of solexa_ids
###############################################################################
...
my $sql = qq~ Select * from $TBST_SOLEXA_SAMPLE ~;
#	my $sql = qq~ 	SELECT distinct (gi.solexa_id) 
#			FROM $TBST_SOLEXA_GENE_INTENSITY gi
#			JOIN $TBST_SOLEXA afa ON (afa.solexa_id = gi.solexa_id)
#			JOIN $TBST_SOLEXA_SAMPLE afs ON (afs.solexa_sample_id = afa.solexa_sample_id)
#			WHERE afs.project_id IN ($project_id) 
#		   ~;
...
#given a project_id return all solexa array ids
#return array of solexa_ids
###############################################################################
...
my $sql = qq~ Select * from $TBST_SOLEXA_SAMPLE ~;
#	my $sql = qq~ 	SELECT afa.solexa_id 
#			FROM $TBST_SOLEXA afa
#			JOIN $TBST_SOLEXA_SAMPLE afs ON (afs.solexa_sample_id = afa.solexa_sample_id)
#			WHERE afs.project_id IN ($project_id) 
#		   ~;
...
###############################################################################
# get_solexa_intensity_data_sql
#Given some id, some optional constriants, and annotation display type make a sql query to display the gene intensity values
#return a sql query
###############################################################################
sub get_solexa_intensity_data_sql {
	my $method = 'get_solexa_intensity_data_sql';
	my $self = shift;
	my %args = @_;
#  $log->printStack( 'debug' );
	
	my $solexa_ids = $args{solexa_ids};
	...
	confess(__PACKAGE__ . "::$method Need to provide arugments 'solexa_ids' you gave '$solexa_ids'\n") unless ($solexa_ids);
    
	my $add_solexa_db_link_table = '';
	
	
	for (my $i; $i<=$#all_constraints; $i++){ 
		my $constraint = $all_constraints[$i];
		
		if ($constraint =~ /link\.db_id/){			#only add the solexa_db_links table if we have a constraints for it 
			$add_solexa_db_link_table = "JOIN $TBST_DBXREF link ON (anno.solexa_annotation_id = link.solexa_annotation_id)";
...
#      sample_group_name
#			FROM $TBST_SOLEXA_GENE_INTENSITY gi 
#			JOIN $TBST_SOLEXA afa ON (gi.solexa_id = afa.solexa_id)
#			JOIN $TBST_SOLEXA_SAMPLE afas ON (afa.solexa_sample_id = afas.solexa_sample_id)
#			JOIN $TBST_SOLEXA_ANNOTATION anno ON (anno.probe_set_id = gi.probe_set_id)
#			JOIN $TBST_SOLEXA_ANNOTATION_SET anno_s ON (anno.solexa_annotation_set_id = anno_s.solexa_annotation_set_id) $add_solexa_db_link_table
#			WHERE anno_s.solexa_annotation_set_id = $annotation_set_id AND 
#			gi.protocol_id = $R_protocol_id AND
#			gi.solexa_id IN ($solexa_ids) 
#			$constraints
...
sub delete_analysis_session {
	...
	my $returned_id = $sbeams->updateOrInsertRow(
							table_name=>'fake',  # $TBST_SOLEXA_ANALYSIS,
				   			rowdata_ref=>$rowdata_ref,
				   			return_PK=>1,
				   			verbose=>'',
				   			testonly=>'',
				   			update=>1,
				   			PK_name => 'solexa_analysis_id',
...
sub delete_analysis_folder {
	...
	my $base_folder_path = $self->solexa_bioconductor_delivery_path();
...
#Retrun solexa_analysis_id for success 0 if it did not work
###############################################################################
...
my $sql = qq~ select * from $TBST_SOLEXA_SAMPLE ~;
#	my $sql = qq~ SELECT solexa_analysis_type_id
#				  FROM $TBST_SOLEXA_ANALYSIS_TYPE
#				  WHERE solexa_analysis_name like '$analysis_name_type'
#				~;
	
	#$sbeams->display_sql(sql=>$sql);
				
	my @solexa_analysis_ids = $sbeams->selectOneColumn($sql);
	
	
	if (@solexa_analysis_ids) {
		return $solexa_analysis_ids[0];
	}else{
		return 0;
	}
}
...
my $sql = qq~ select solexa_sample_id from $TBST_SOLEXA_SAMPLE ~;
...
#				~;
	...
	my @solexa_folder_names = $sbeams->selectOneColumn($sql);
	
	
	if (@solexa_folder_names) {
		return $solexa_folder_names[0];
	}else{
		return 0;
	}
}
...
my $sql = qq~ select * from $TBST_SOLEXA_SAMPLE ~;
#	my $sql = qq~ SELECT aa.solexa_analysis_id, 
#				 aa.folder_name,
#				 aa.user_description, 
#				 aa.analysis_description,
#				 aa.parent_analysis_id, 
#				 aat.solexa_analysis_name, 
#				 aa.date_created,
#				 ul.username
#				 FROM $TBST_SOLEXA_ANALYSIS aa
#				 JOIN 	$TBST_SOLEXA_ANALYSIS_TYPE aat ON (aa.solexa_analysis_type_id = aat.solexa_analysis_type_id)
...
#				~;
	...
	if ($data[0]){
		...
		my $analysis_data_o = new SBEAMS::SolexaTrans::Analysis_Data(data=>\@data);
...
#				~;
	...
	my @solexa_project_id = $sbeams->selectOneColumn($sql);
	
	
	if (@solexa_project_id) {
		return $solexa_project_id[0];
	}else{
		return 0;
	}
}
...
my $sql = qq~ select * from $TBST_SOLEXA_SAMPLE ~;		
#	my $sql = "SELECT * FROM $TBST_SOLEXA_ANALYSIS WHERE folder_name = '$folder_name'";
...
#		FROM $TBST_SOLEXA afa 
#		JOIN $TBST_SOLEXA_SAMPLE afs ON (afa.solexa_sample_id = afs.solexa_sample_id) 
...
#					  FROM $TBST_SOLEXA_SAMPLE afs
#					  JOIN $TBST_SOLEXA afa ON (afa.solexa_sample_id = afs.solexa_sample_id)
