...
vq.ScatterPlot.prototype._setOptionDefaults = function(options) {
        // PUBLIC OPTIONS
        // padding
        if (options.selectedProbesetId)  {  this._selectedProbesetId = options.selectedProbesetId; }
...
vq.ScatterPlot.prototype.draw= function(data) {
        ...
        if (this.data.isDataReady()) {
            ...
            var valid_data = data_array.filter(function(d,e,f) { return (d[y] && d[x]); }),
            sum_x = pv.sum(valid_data, function(d) { return d[x]; }),
            sum_y = pv.sum(valid_data, function(d) { return d[y]; }),
            sum_x2 = pv.sum(valid_data, function(d) { return d[x] * d[x]; }),
            //sum_y2 = pv.sum(valid_data, function(d) { return d[y] * d[y]; }),
            sum_xy = pv.sum(valid_data, function(d) { return d[x] * d[y]; }),
           ...
           var panel = vis.add(pv.Panel)
            ...
            panel.add(pv.Line)
                    ...
                    .bottom(function(d) { return yScale(lineArray(d));})
                    .strokeStyle("#0b0");
               }

            var strokeStyle = function(data) { return pv.color(dataObj._strokeStyle(data));};
            var fillStyle = function(data) { return pv.color(dataObj._fillStyle(data));};
                panel.add(pv.Dot)
                    .def("active", -1)
                    .data(data_array)
...
vq.models.ScatterPlotData.prototype.setDataModel = function () {
    this._dataModel = [
            ...
            {label : '_fillStyle', id: 'fill_style',cast :vq.utils.VisUtils.wrapProperty,
                defaultValue : function() { return pv.color('steelblue').alpha(0.2);}},
            {label : '_strokeStyle', id: 'stroke_style',
                 cast :vq.utils.VisUtils.wrapProperty, defaultValue : function() { return 'steelblue';}},
            {label : '_radius', id: 'radius',cast :vq.utils.VisUtils.wrapProperty, defaultValue : function() { return 2;}},
            {label : '_shape', id: 'shape',cast : vq.utils.VisUtils.wrapProperty, defaultValue : function() { return 'dot';}},
