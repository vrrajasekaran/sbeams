...
var sampleClass = function(settings){
    ...
    paramsObj.filtercol = jQuery('select#selectedTitle').val();
		...
		Ext.Ajax.request(
		{
      url: common.pathname + '/cgi/Repository/query_repository.cgi',
			params: paramsObj,
			success: function ( result, request ) 
			{
				var obj = '';
	...
	function style_metaData(d,userT){
		...
		for(i = 0; i<record.length; i++){
			...
			details += common.checkIfExists(data.taxonomy)?'<div id="box">'+openTable+"<b>Organism:</b> "+inbetweenTD+org+closeTable+'</div>': '';
      ...
      details += common.checkIfExists(data.summary)? "<h3>Summary</h3><div id='box'>"+data.summary+"</div>" : '';
			...
			details += common.checkIfExists(data.instrumentation)? "<h3>Instrumentation</h3><div id='box'>"+common.print_r(data.instrumentation,{})+"</div>" : '';
  ...
  var getPubmedCitation = function (arr,id){
    ...
    if(typeof arr == 'object'){
      ...
      Ext.Ajax.request({
        url: common.pathname + '/cgi/Repository/pmqproxy.cgi',
        params: {ids: ids},
        success: function ( result, request ){
	...
	var SampleRecord = Ext.data.Record.create([
    	{ name: 'acc', mapping: 'acc' },      
    	{ name: 'title' },
      { name: 'sampletag' },
      { name: 'proteins' ,mapping: 'counts.proteins' },
    	{ name: 'peptides' ,mapping: 'counts.peptides'},
    	{ name: 'spectra'  ,mapping: 'counts.spectra'},
    	{ name: 'taxonomy' ,mapping: 'taxonomy.name'}, //,convert: render_organism },
    	{ name: 'deposit_date' ,mapping: 'dates.deposit.std'},
    //	{ name: 'release',mapping: 'dates.release.std', sortType: 'asDate' },
    //	{ name: 'metadata_update_date' ,mapping: 'dates.metadata.std'},
		//{ name: 'data_date' ,mapping: 'dates.data.std'},
		{ name: 'data_date' ,mapping: 'dates',convert: function (e,v){if(e.data){return e.data.std;}else{return undefined;}}},
    	{ name: 'platform', mapping: 'instrumentation.platform'},     // Platform title
		{ name: 'contact' },
		{ name: 'preparation' },
    ...
    { name: 'summary' },
    { name: 'publication', mapping: 'publication'},
    { name: 'contributors'},
		{ name: 'data_processing' },	
		{ name: 'xrefs', convert: common.xrefs_renderer },
		{ name: 'other_parameters',mapping: 'dates',convert: function (e,v){if(v.other_parameters){return v.other_parameters;}else{return undefined;}} },
		{ name: 'description',mapping: 'dates',convert: function (e,v){if(v.description){return v.description;}else{return undefined;}} },
		{ name: 'public' },
	...
	function make_grid( details ){
		...
		[
			sample_expander,
			{ 
				header: "Accession", width: 55, fixed: false, dataIndex: 'acc',  
				tooltip: 'Sample Accession' 
			},
			{ 
				header: "Sample Tag", dataIndex: 'sampletag', width: 100, 
				tooltip: 'Sample Tag' 
			},
      {
        header: "Sample Title", dataIndex: 'title', width: 190,
    ...
    var button = jQuery('input#downloadbtn').click(function(){
        ...
        var p = {cmd: 'download', filterstr: filterstr, filtercol: filtercol , dlfiletype: dlfiletype};
        var paramsObj = common.setParamsObj( p );
        var request = common.pathname + '/cgi/Repository/query_repository.cgi?cmd=download&filterstr='+filterstr+'&filtercol='+filtercol+'&dlfiletype='+dlfiletype;
    ...
    var button = jQuery('input#downloadbtn2').click(function(){
        var filterstr = jQuery('input#searchq').val();
        var filtercol = jQuery('select#selectedTitle').val();

        var p = {cmd: 'downloadtable', filterstr: filterstr, filtercol: filtercol};
        var paramsObj = common.setParamsObj( p );
        var request = common.pathname + '/cgi/Repository/query_repository.cgi?cmd=downloadtable&filterstr='+filterstr+'&filtercol='+filtercol;
        var iframe = Ext.DomHelper.append(document.body, {
                tag: 'iframe',
                frameBorder: 0,
                width: 0,height: 0,
