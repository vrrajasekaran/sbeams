...
sub main { 
    ...
    croak "This TABLE_NAME=$TABLE_NAME cannot be managed by this program."
      unless ($sbeamsIJ->returnTableInfo($TABLE_NAME,"ManageTableAllowed"))[0] eq "YES";
...
sub processEntryForm {
    ...
    my ($multi_insert_column) = 
      $sbeamsIJ->returnTableInfo($TABLE_NAME,"MULTI_INSERT_COLUMN");
    ...
    # Otherwise, the action is INSERT, so build a SQL statement for that
        ...
        my @key_columns = 
          $sbeamsIJ->returnTableInfo($TABLE_NAME,"key_columns");
        my %unique_values;
        if (@key_columns) {
