...
sub print_download_file_page {
  ...
  if ( $parameters{'edited_info_file'} ne "" ) {
    ...
    $edited_text = parse_excel_sample_info_file($fh);
  }
  else {
  ...
  ## If there are no parsing errors, go ahead with GEO file creation
    ...
    my $token                = $parameters{'token'};
    my $expression_data_file = "$BC_UPLOAD_DIR/$token/${token}_annotated.txt";
    my $expression_data_ref = parse_expression_data_file($expression_data_file);
