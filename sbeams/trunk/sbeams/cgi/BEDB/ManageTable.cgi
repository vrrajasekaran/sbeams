...
use strict;
use lib qw (../../lib/perl);
use vars qw ($q $sbeams $sbeamsBE $dbh $current_contact_id $current_username
...
sub main { 
    ...
    croak "This TABLE_NAME=$TABLE_NAME cannot be managed by this program."
      unless ($sbeamsBE->returnTableInfo($TABLE_NAME,"ManageTableAllowed"))[0] eq "YES";

    ($CATEGORY) = $sbeamsBE->returnTableInfo($TABLE_NAME,"CATEGORY");
    ($PROGRAM_FILE_NAME) = $sbeamsBE->returnTableInfo($TABLE_NAME,"PROGRAM_FILE_NAME");
    ($DB_TABLE_NAME) = $sbeamsBE->returnTableInfo($TABLE_NAME,"DB_TABLE_NAME");
    ($PK_COLUMN_NAME) = $sbeamsBE->returnTableInfo($TABLE_NAME,"PK_COLUMN_NAME");
    @MENU_OPTIONS = $sbeamsBE->returnTableInfo($TABLE_NAME,"MENU_OPTIONS");
...
sub printOptions {
    ...
    $sbeamsBE->printPageFooter("CloseTables");
...
sub printEntryForm {
    ...
    my @columns = $sbeamsBE->returnTableInfo($TABLE_NAME,"ordered_columns");
    my %input_types = 
      $sbeamsBE->returnTableInfo($TABLE_NAME,"input_types");
...
sub showTable {
    ...
    my ($main_query_part) =
      $sbeamsBE->returnTableInfo($TABLE_NAME,$detail_level."Query",
      $parameters_ref);
    ...
    my ($element,$value);
    my %url_cols = $sbeamsBE->returnTableInfo($TABLE_NAME,"url_cols");
...
sub processEntryForm {
    ...
    my ($multi_insert_column) = 
      $sbeamsBE->returnTableInfo($TABLE_NAME,"MULTI_INSERT_COLUMN");
    ...
    my @required_columns = 
      $sbeamsBE->returnTableInfo($TABLE_NAME,"required_columns");
    if (@required_columns) {
      my $error_message;
    ...
    my @data_columns = 
      $sbeamsBE->returnTableInfo($TABLE_NAME,"data_columns");
    my %input_types = 
      $sbeamsBE->returnTableInfo($TABLE_NAME,"input_types");
    ...
    # Otherwise, the action is INSERT, so build a SQL statement for that
        ...
        my @key_columns = 
          $sbeamsBE->returnTableInfo($TABLE_NAME,"key_columns");
        my %unique_values;
        if (@key_columns) {
