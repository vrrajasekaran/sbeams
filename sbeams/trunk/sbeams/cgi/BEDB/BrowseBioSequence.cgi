...
sub printEntryForm {
    ...
    $TABLE_NAME="BE_BrowseBioSequence" unless ($TABLE_NAME);
    ($PROGRAM_FILE_NAME) =
    ...
    $sql = qq~
      SELECT $limit_clause $columns_clause
        FROM $TBBE_BIOSEQUENCE BS
        LEFT JOIN $TBBE_BIOSEQUENCE_SET BSS ON ( BS.biosequence_set_id = BSS.biosequence_set_id )
       WHERE 1 = 1
