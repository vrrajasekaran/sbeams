...
sub main { 
    ...
    croak "This TABLE_NAME=$TABLE_NAME cannot be managed by this program."
      unless ($sbeamsPH->returnTableInfo($TABLE_NAME,"ManageTableAllowed"))[0] eq "YES";
...
sub showTable {
    ...
    my ($main_query_part) =
      $sbeamsPH->returnTableInfo($TABLE_NAME,$detail_level."Query",
      $parameters_ref);
    ...
    my ($element,$value);
    my %url_cols = $sbeamsPH->returnTableInfo($TABLE_NAME,"url_cols");
...
sub processEntryForm {
    ...
    my ($multi_insert_column) = 
      $sbeamsPH->returnTableInfo($TABLE_NAME,"MULTI_INSERT_COLUMN");
    ...
    my @required_columns = 
      $sbeamsPH->returnTableInfo($TABLE_NAME,"required_columns");
    if (@required_columns) {
      my $error_message;
    ...
    my @data_columns = 
      $sbeamsPH->returnTableInfo($TABLE_NAME,"data_columns");
    my %input_types = 
      $sbeamsPH->returnTableInfo($TABLE_NAME,"input_types");
    ...
    # Otherwise, the action is INSERT, so build a SQL statement for that
        ...
        my @key_columns = 
          $sbeamsPH->returnTableInfo($TABLE_NAME,"key_columns");
        my %unique_values;
        if (@key_columns) {
