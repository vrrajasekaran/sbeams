...
###############################################################################
$PROGRAM_FILE_NAME = 'SampleTags.cgi';
...
sub print_sample_info_tab {
	...
	my $base_url = "$CGI_BASE_DIR/SolexaTrans/SampleTags.cgi";
  ...
  ##Start looking for data that can be run with the SolexaTrans pipeline
		...
		<TABLE WIDTH="66%" BORDER=0>
		  <TR>
		   <TD>
        <B>
        This page displays every job for every sample and the number of tags found in each job.
        </B> 
       </TD></TR>
       <TR><TD></TD></TR>
		</TABLE>
		...
		if ($display_type eq 'SOLEXA_SAMPLE'){
			...
			$sbeams_solexa_groups->setSBEAMS($sbeams);
                        ...
                        if ($parameters{'filter_jobs_slimseq_sample_id'}) {
                          $sql = $sbeams_solexa_groups->get_slimseq_sample_info_sql(project_id => $project_id,
...
sub append_job_link {
	...
	foreach my $row_aref (@{$aref} ) {
	  ...
	  my $slimseq_sample_id  = $row_aref->[$id_idx];		
          ...
          push @$row_aref, $view_params;		#append on the new data	
	} # end foreach row
		
        if ( 1 ){
	    #need to add the column headers into the resultset_ref since DBInterface display results will reference this
	    push @{$resultset_ref->{column_list_ref}} , "Job Info"; 
